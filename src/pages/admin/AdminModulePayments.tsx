    ...
    <div className="p-4 md:p-8 space-y-6 max-w-5xl mx-auto w-full max-w-full overflow-x-hidden">
      <div className="flex items-start gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground break-words">Analise comprovantes e aprove ou rejeite pagamentos de add-ons.</p>
        </div>
      </div>
            ...
            {filtered.map((p) => (
              <div key={p.id} className="rounded-xl border border-border bg-card p-4 space-y-3">
                <div className="space-y-1 min-w-0">
                  <p className="font-medium text-sm truncate">{p.user_nome}</p>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm">{MODULE_LABELS[p.module_name] ?? p.module_name}</p>
                    <p className="text-sm font-semibold mt-1">R$ {p.amount.toFixed(2)}</p>
                  </div>
                  <div className="shrink-0">{statusBadge(p)}</div>
                </div>
                <p className="text-[11px] text-muted-foreground">{format(new Date(p.submitted_at), "dd/MM/yyyy HH:mm")}</p>
                {p.receipt_url && (
                  <a href={p.receipt_url} target="_blank" rel="noopener noreferrer" className="inline-flex text-primary text-xs items-center gap-1 hover:underline">
                ...
                <div className="flex flex-wrap gap-2">
                  {p.status === "pending_review" && (
                    <>
                      <Button size="sm" variant="default" className="rounded-xl text-xs flex-1 min-w-[120px]" onClick={() => { setActionTarget(p); setActionType("approve"); setReason(""); }}>
                        <CheckCircle2 className="h-3.5 w-3.5 mr-1" /> Aprovar
                      </Button>
                      <Button size="sm" variant="outline" className="rounded-xl text-xs text-destructive border-destructive/30 flex-1 min-w-[120px]" onClick={() => { setActionTarget(p); setActionType("reject"); setReason(""); }}>
                        <XCircle className="h-3.5 w-3.5 mr-1" /> Rejeitar
                      </Button>
                    </>
                  )}
                  <Button size="sm" variant="ghost" className="rounded-xl text-xs text-destructive hover:text-destructive hover:bg-destructive/10 px-3" onClick={() => setDeleteTarget(p)}>
