...
import { QrCode, Save, Copy } from "lucide-react";
...
export default function AdminPix() {
  ...
  const [form, setForm] = useState({
    ...
    cidade_beneficiario: "", copia_cola: "",
  });
    ...
    const load = async () => {
      ...
      if (data && data.length > 0) {
        ...
        setForm({
          ...
          nome_beneficiario: c.nome_beneficiario || "", cidade_beneficiario: c.cidade_beneficiario || "",
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      nome_beneficiario: form.nome_beneficiario, cidade_beneficiario: form.cidade_beneficiario,
      copia_cola: form.copia_cola,
    };
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(form.copia_cola);
      ...
      <div className="rounded-2xl bg-card border border-border p-6 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            <Input value={form.copia_cola} onChange={(e) => setForm({ ...form, copia_cola: e.target.value })} className="h-10 bg-secondary/50 border-border flex-1" placeholder="Cole o código Pix copia e cola aqui" />
            <Button variant="outline" size="icon" className="h-10 w-10 shrink-0" onClick={copyToClipboard} disabled={!form.copia_cola}>
