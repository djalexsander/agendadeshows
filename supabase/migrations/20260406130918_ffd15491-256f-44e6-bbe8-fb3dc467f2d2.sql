...
BEGIN
  ...
  VALUES (
    ...
    CASE 
      WHEN v_is_self_signup THEN 'aguardando_pagamento'
      ELSE 'ativo'
    END,
    CASE
  ...
  IF v_is_self_signup THEN
    ...
    VALUES (
      'novo_cadastro',
      'Novo cadastro',
      'O usuário ' || COALESCE(NEW.raw_user_meta_data->>'nome', NEW.email) || ' se cadastrou. Valor: R$ ' || COALESCE(v_valor_padrao, 0)::text,
      NEW.id
    );
  END IF;
