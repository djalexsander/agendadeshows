...
ALTER TABLE public.profiles ADD COLUMN grace_ends_at timestamp with time zone DEFAULT NULL;
...
BEGIN
  ...
  INSERT INTO public.profiles (user_id, email, nome, telefone, cidade, estado, status_plano, origem_cadastro, valor_plano, valor_padrao_na_data, plan_type, is_paid, trial_started_at, trial_ends_at, grace_ends_at)
  VALUES (
    NEW.id,
    COALESCE(NEW.email, ''),
